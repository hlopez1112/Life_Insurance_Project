...
def load_mortality_files(data_dir: Path) -> pd.DataFrame:
    """
    Load and combine all mortality tables.
    """
    dfs = []
    
    for filename, risk_class, gender in FILES:

        file_path = data_dir / filename
        df = read_file( path=file_path, risk_class=risk_class, gender=gender)
        dfs.append(df)
